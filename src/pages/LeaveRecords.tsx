...
export default function LeaveRecords() {
  ...
  const fetchLeaveRecords = useCallback(async () => {
    try {
      ...
      if (selectedStatus !== 'all') query = query.eq('status', selectedStatus as 'pending' | 'approved' | 'rejected');
  ...
  useEffect(() => {
    (async () => {
      ...
      setLeaveTypeNames(Object.fromEntries(types.map((t) => [t.id, t.name])));
    })();
  }, []);

  // Per-type balances for the staff member in scope.
  useEffect(() => {
    let cancelled = false;
    if (!balanceStaffId) {
      setBalances([]);
  ...
  const unpaidPenaltyCount = leaveRecords.filter(
    (r) => r.status === 'approved' && (r.leave_type === 'unpaid' || r.leave_type === 'penalty'),
  ).length;
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <CardContent>
            <div className={cn('text-2xl font-bold', pendingCount > 0 && 'text-warning')}>{pendingCount}</div>
            <p className="text-xs text-muted-foreground mt-0.5">{isStaff ? 'Waiting' : 'Need action'}</p>
          </CardContent>
        </Card>
      </div>

      {/* Per-type leave balances for the staff in scope */}
      {balanceStaffId && balances.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {balances.map((b) => (
                <div key={b.type.id} className="flex items-center justify-between gap-3 rounded-lg border p-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-muted-foreground">
                      {b.type.accrual === 'none' ? `${b.used} used` : `${b.used} used of ${b.accrued}`}
                    </p>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className={cn('text-lg font-bold tabular-nums', b.type.accrual !== 'none' && b.balance < 0 && 'text-destructive')}>
                      {b.type.accrual === 'none' ? '—' : b.balance}
                    </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            {!isStaff && (
              <Select value={selectedStaffId} onValueChange={setSelectedStaffId}>
                <SelectTrigger className="w-full md:w-40 h-9"><SelectValue placeholder="All Staff" /></SelectTrigger>
                <SelectContent>
                ...
                <TableBody>
                  {filteredRecords.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{typeLabel(record)}</Badge>
                      </TableCell>

                      <TableCell className="text-center">
                        {isStaff ? (
                          <span className={cn('text-sm font-medium', record.deduction_days > 0 ? 'text-destructive' : 'text-success')}>
                      ...
                      <TableCell>
                        {isStaff ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className={cn('text-sm', record.status === 'pending' && 'text-warning', record.status === 'approved' && 'text-success', record.status === 'rejected' && 'text-destructive')}>
                              {record.status === 'pending' ? 'Waiting' : record.status === 'approved' ? 'Approved' : 'Declined'}
                            </span>
                          </div>
                        ) : (
                      ...
                      {!isStaff && (
                        <TableCell className="text-sm text-muted-foreground">{record.approved_by ? 'Manager' : '-'}</TableCell>
                      )}
