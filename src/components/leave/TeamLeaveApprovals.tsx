...
export function TeamLeaveApprovals() {
  ...
  const [records, setRecords] = useState<LeaveRequestGroup[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<LeaveRequestGroup | null>(null);
  ...
  const fetch = useCallback(async () => {
    ...
    if (!error) {
      ...
      // One entry per request, matching the Approve Leave page — a manager
      // should not be shown a week off as five separate things to review.
      setRecords(groupLeaveRequests(reports));
    }
    setLoading(false);
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-2">
          {records.map((r) => (
            <div key={r.ids.join(',')} className="flex items-center gap-3 rounded-md border p-2.5">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">{r.staff?.full_name}</p>
                <p className="flex items-center gap-1 text-xs text-muted-foreground">
                  <CalendarClock className="h-3 w-3" />
                  {r.ids.length > 1
                    ? `${r.dateLabel} · ${r.ids.length} days`
