-- Create admin function to clear transaction data (bypasses immutability triggers)
-- This uses session_replication_role to temporarily disable triggers
...
BEGIN
  ...
  -- 5. Delete payment_requests (by created_at)
  DELETE FROM public.payment_requests
  WHERE created_at >= _date_from AND created_at <= (_date_to + INTERVAL '1 day');
  ...
  deleted_counts := jsonb_build_object(
    ...
    'expenses', exp_count,
    'total', jl_count + je_count + le_count + ss_count + pr_count + exp_count
  );
