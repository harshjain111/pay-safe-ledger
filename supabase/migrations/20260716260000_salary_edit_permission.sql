-- ============================================================================
-- Let admins manage salaries (assign / increment), not just owners.
--
-- Introduces the salaries.edit capability, grants the Administrator template
-- salaries.view + salaries.edit, and re-gates bulk_update_salaries on
-- has_permission('salaries.edit') (owners still short-circuit true). The client
-- gates the salary UI on the same capability.
-- ============================================================================

-- Administrator can now see + edit salaries.
update public.rights_templates
set permissions = (
  select array(select distinct p from unnest(permissions || array['salaries.view','salaries.edit']) as p)
)
where name = 'Administrator';

-- Re-gate the bulk update: owner OR anyone granted salaries.edit.
...
begin
  if not public.has_permission(v_caller, 'salaries.edit') then
    raise exception 'Not authorized to change salaries';
  end if;
  ...
  return jsonb_build_object(
    'updated', v_updated, 'increments', v_incr, 'decrements', v_decr,
    'net_delta', v_net, 'changes', v_changes
  );
end;
$$;
